...
#' @param Lt A list of n vectors containing the observation time points for every individual, such that the ith element of the list comprises the num.years.i points in the direction of (year) dimension 1 at which the functional-valued stochastic process is observed for the ith individual.
#' @param Ly A list of n matrices containing the observed values for every individual, such that the ith element is an num.ages by num.years.i matrix of observed values for the ith individual.
#' @param fpca.op1 A list of options control parameters specified by \code{list(name=value)} for FPCA in direction of (age) dimension 2; check fdapace::FPCA for available control options and default settings.
#' @param fpca.op2 A list of options control parameters specified by \code{list(name=value)} for FPCA in direction of (year) dimension 1; check fdapace::FPCA for available control options and default settings.
...
SparseMarginalFPCA <- function(sSup, Lt, Ly, fpca.op1 = list(), fpca.op2 = list(), pc.j = NULL, pc.k = NULL, bw_mu_min=NULL, bw_mu_max=NULL){
  ...
  if(!all(ni_y[2,] == ni_tt)){
    stop("SparseMarginalFPCA is aborted because the length of Lt and column numbers of Ly do not match!\n");
  }
  if(!all(ni_y[1,] == n_sSup)){
    stop("SparseMarginalFPCA is aborted because the length of sSup and row numbers of Ly do not match!\n");
  }
  if( !all(unlist(lapply(Ly,function(x) typeof(x) %in% c('integer', 'double') ) ) ) ){
    stop("SparseMarginalFPCA is aborted because 'Ly' members are not all of type double or integer! Try  \"lapply(y,function(x) typeof(x))\" to see the current types. \n");
  }
  if( !all(unlist(lapply(Lt,function(x) typeof(x) %in% c('integer', 'double'))) ) ){
    stop("SparseMarginalFPCA is aborted because 'Lt' members are not all of type double or integer! Try  \"lapply(t,function(x) typeof(x))\" to see the current types. \n");
  }

  if(any( unlist( lapply(Lt, function(x) length(x) != length(unique(x))))) ){
    stop("SparseMarginalFPCA is aborted because within-subject 'Lt' members have duplicated values.  Try  \"which( unlist( lapply(t, function(x) length(x) != length(unique(x)))))\" to see potentially problematic entries. \n");
  }
  ...
  if(!is.null(bw_mu_min)){
    if(!typeof(bw_mu_min) %in% c('integer', 'double')){
      stop("SparseMarginalFPCA is aborted because bw_mu_minis not the type double or integer! \n");
    }
  }
  ...
  for(i in 1:length(sSup)){ ### ith row corresponding to jth age in sSup
    ...
    bw_mu <- CVLwls1D(Lyy, Ltt, kernel = "epan", npoly=1, nder=0, dataType= "sparse", kFolds = 5,
                      useBW1SE = FALSE)
    ...
    if(is.null(bw_mu_min)==FALSE)
    {
      ...
      else{warning(sprintf("Warning: The minimum bandwidth should be greater than 0"))}
    }

    if(is.null(bw_mu_max)==FALSE)
    {
      if(bw_mu_max>0){bw_mu_final <- min(bw_mu_fin,bw_mu_max)}
      else{warning(sprintf("Warning: The maximum bandwidth should be greater than 0"))}
    }
  ...
  warning(sprintf("Bandwidth for mean estimation: The bandwidth chosen for mean function estimation is %f",bw_mu_final))
  ...
  # step 3
  phi <- vector('list', length = pc.s)
  xiEst <- vector('list', length = pc.s)
  Lambda <- vector('list', length = pc.s)
  pc.t <- numeric(pc.s)
  obsGrid <- sort(unique(t.n))

  if( is.null(fpca.op2$dataType)==FALSE){if(fpca.op2$dataType == 'Dense'){warning(sprintf("Warning: Along dimension 1, data type is sparse"))}}
  if( is.null(fpca.op2$error)==FALSE){if(fpca.op2$error == 'TRUE'){warning(sprintf("Warning: Along dimension 1, data type is sparse and thus it is better to assume measurement error to be FALSE in order to avoid unstable estimates for sparse data. Hence changed to FALSE"))}}
  if(is.null(fpca.op2$error)== TRUE){warning(sprintf("Warning: Along dimension 1, data type is sparse and thus it is better to assume measurement error to be FALSE in order to avoid unstable estimates for sparse data. Hence error= FALSE is used"))}
  if( is.null(fpca.op2$userMu)==FALSE){warning(sprintf("Warning: Along dimension 1, FPCA does not accept user defined mean since the function internally decentralizes using bivariate mean function"))}
  ...
  #tListt <- lapply(1:n, function(i) t.n[c((t.n_cum[i]+1): t.n_cum[i+1])]) ###Extracts years for ith individual ##But that's Lt[[i]]

  for (j in 1:pc.s){
    yList <- lapply(1:n, function(i) xiEst.s[c((t.n_cum[i]+1): t.n_cum[i+1]),j]) ### Extracts Xi_j(t) for the ith individual
    tList <- Lt
    ...
    res <- fdapace::FPCA(yList, tList, optns = fpca.op2) ##FPCA in year dimension
    pc.t_max <- pc.k[j]
    if (is.null(pc.t_max)){
      pc.t[j] <- dim(res$xiEst)[2] ###pc.k value: default value based on FVE
    } else {
      pc.t[j] <- min(pc.t_max,dim(res$xiEst)[2]) ###pc.k value: based on user input
      if(pc.t_max> dim(res$xiEst)[2]){
        warning(sprintf("Warning: Desired number of components %d exceeds the maximum number of components %d returned by FPCA",
                        pc.t_max, dim(res$xiEst)[2]))
      }
    }
    phi[[j]] <- res$phi[, 1:pc.t[j]]
    xiEst[[j]] <- res$xiEst[, 1:pc.t[j]]
    Lambda[[j]] <- res$lambda[1:pc.t[j]]
  }
  ### calculate FVE
